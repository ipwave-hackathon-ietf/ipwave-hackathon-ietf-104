...
//    typedef Ipv6NeighbourCache::MultihopDadEntry MultihopDadEntry;    // for convenience
...
//    Ipv6NeighbourCache dadTable;
    ...
    //XIANG ZHONG
//    struct MultihopDadEntry
//    {
//        int interfaceId;    // interface on which DAD is performed
//        Ipv6Address address;    // link-local address subject to DAD
//        MultihopDadEntry(Ipv6Address addr, int ifaceID) { address = addr; interfaceId = ifaceID; }
//        bool operator<(const MultihopDadEntry& b) const
//        {
//            return interfaceId == b.interfaceId ? address < b.address : interfaceId < b.interfaceId;
//        }
//
////        MacAddress linkLocalAddress;
////        int numNSSent;    // number of DAD solicitations sent since start of sim
////        cMessage *timeoutMsg;    // the message to cancel when NA is received
//    };


//    typedef std::set<MultihopDadEntry *> MultihopDadList;
//    typedef std::map<Key,Neighbour> MultihopDadMap;

    typedef std::map<Key,Neighbour> TestMap;
    //END
    ...
    //XIANG ZHONG
    struct AddrRegistEntry
        {
            int interfaceId;    //interface on which Router Discovery is performed
            unsigned int numNSSent;    //number of Router Solicitations sent since start of sim
            cMessage *timeoutMsg;    //the message to cancel when RA is received
        };
    typedef std::set<AddrRegistEntry *> AddrRegistList;
    AddrRegistList addrRegistList;
  ...
  protected:
    ...
    //XIANG ZHONG
    virtual Ipv6NeighbourDiscovery::AddrRegistEntry *fetchAddrRegistEntry(InterfaceEntry *ie);
    ...
    virtual void initiateMultihopDad(const Ipv6Address& tentativeAddr, Ipv6Address& destAddr,InterfaceEntry *ie);
    virtual void createAndSendDARPacket(const Ipv6Address& dadTargetAddr, const Ipv6NeighbourSolicitation *ns, const Ipv6Address& destAddr,
            const Ipv6Address& srcAddr, InterfaceEntry *ie);
    virtual void processDARPacket(Packet *packet, const Ipv6DuplicateAddrReq *dar);
    virtual void createAndSendDACPacket( const Ipv6Address& dgDestAddr,
            const Ipv6Address& dgSrcAddr, const Ipv6DuplicateAddrReq *dar, int status, InterfaceEntry *ie);
    virtual void processDACPacket(Packet *packet, const Ipv6DuplicateAddrConfirm *dac);
    virtual void processNsForMultihopDAD(Packet *packet,
            const Ipv6NeighbourSolicitation *ns, InterfaceEntry *ie);
    virtual void sendSolicitedNaForDAD(Packet *packet, Ipv6Address& destAddr, const Ipv6DuplicateAddrConfirm *dac,
            InterfaceEntry *ie);
    virtual bool validateDAR(Packet *packet, const Ipv6DuplicateAddrReq *dar);
    virtual bool validateDAC(Packet *packet, const Ipv6DuplicateAddrConfirm *dac);
    virtual bool validateARO(Ipv6NdAddressRegistrationOption *aro);
    virtual void processAddrRegistRetransmit(cMessage *msg);
    virtual void processAddrRegistTimeout(cMessage *msg);
    virtual void processNsForUpdateNCE(Packet *packet, const Ipv6NeighbourSolicitation *ns, InterfaceEntry *ie);
    virtual void forwardNDMessageForNeighbour(Packet *packet, const Ipv6Address& destAddr, InterfaceEntry *ie);
    virtual void initiateAddressRegistration(const Ipv6Address& myIpv6Addr, Ipv6Address& destAddr, InterfaceEntry *ie);
