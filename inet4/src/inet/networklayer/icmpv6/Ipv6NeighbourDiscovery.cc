        ...
        const Ipv6Address& srcAddr, int interfaceId)
{
    ...
    //XIANG ZHONG
//    simtime_t delay = uniform(0.001,0.02);
    simtime_t delay = uniform(0.000001,0.00002); //1us to 20us
//    send(msg, "ipv6Out");
...
void Ipv6NeighbourDiscovery::assignLinkLocalAddress(cMessage *timerMsg)
{
    ...
    for (int i = 0; i < ift->getNumInterfaces(); i++) {
        ...
        //XIANG ZHONG
        //Assign global address for router in initiate
        if (rt6->isRouter()&& !(ie->isLoopback())) {
            ...
            if (strncmp("nic", ie->getInterfaceName(), 3) == 0) {
                ...
                EV_INFO<< "Router " << ie->getInterfaceName()<<" global address:"<<ie->ipv6Data()->getAddress(0) <<endl;
            }
...
//    Ipv6Address destAddr = tentativeAddr.formSolicitedNodeMulticastAddress();
    // Mannually set RSU's address as destAddr
//    Ipv6Address destAddr = Ipv6Address("fe80::8aa:ff:fe00:2");
    ...
    std::cout<< "initiateMulithopDad(): numNSSent: "<<addrRegistEntry->numNSSent << endl;
    ...
    createAndSendNsPacket(tentativeAddr, destAddr, ie->ipv6Data()->getAddress(0), ie);
...
//    nce->nudTimeoutEvent = msg;
    scheduleAt(simTime() + 20 +uniform(0.0001,0.002), msg);
...
void Ipv6NeighbourDiscovery::initiateAddressRegistration(const Ipv6Address& myIpv6Addr, Ipv6Address& destAddr, InterfaceEntry *ie)
{
    ...
    addrRegistEntry->numNSSent = 0;
    addrRegistEntry->numNSSent++;
    std::cout<< "initiateAddressRegistration: numNSSent: "<<addrRegistEntry->numNSSent << endl;
...
//    simtime_t interval = uniform(ie->ipv6Data()->getMinRtrAdvInterval(), ie->ipv6Data()->getMaxRtrAdvInterval());
    ...
    if (addrRegistEntry==nullptr) {
        std::cout <<"addrRegistEntry is null."<< endl;
    }
    ...
    if (addrRegistEntry!=nullptr && addrRegistEntry->numNSSent < 2) {
        ...
        std::cout << "address registration resent address is: "<< destAddr <<endl;

        Ipv6Address tentativeAddr = ie->ipv6Data()->getPreferredAddress();
        Ipv6Address srcAddr = ie->ipv6Data()->getLinkLocalAddress();
        addrRegistEntry->numNSSent++;
        std::cout << "processAddrRegistRetransmit(): numNSSent: "<<addrRegistEntry->numNSSent << endl;
        createAndSendNsPacket(tentativeAddr, destAddr, srcAddr, ie);
       //    advIfEntry->raTimeoutMsg = msg;
        simtime_t nextScheduledTime = simTime() + interval + uniform(0.0001,0.002);
...
void Ipv6NeighbourDiscovery::processAddrRegistTimeout(cMessage *msg)
{
   ...
   simtime_t registLifeTime = nce->registLifeTime;
   if (nceKey == nullptr)
        throw cRuntimeError("The nceKey is nullptr at nce->MAC=%s, isRouter=%d",
                nce->macAddress.str().c_str(), nce->isRouter);
   if (simTime() > nce->registLifeTime)
   {
        neighbourCache.remove(nceKey->address, nceKey->interfaceID);
...
void Ipv6NeighbourDiscovery::processRdTimeout(cMessage *msg)
{
    ...
    if (rdEntry->numRSSent < ie->ipv6Data()->_getMaxRtrSolicitations()) {
        ...
        //XIANG ZHONG
        //Assumption: if no router on-link, look for vehicular neighbors
...
//        Ipv6Address destAddr = Ipv6Address("FF02::1");
        ...
        std::cout <<"Solicited RA destAddr is: "<< destAddr<< endl;
...
void Ipv6NeighbourDiscovery::processRaPacket(Packet *packet, const Ipv6RouterAdvertisement *ra)
{
    ...
    if (ie->ipv6Data()->getAdvSendAdvertisements()) {
        ...
        //XIANG ZHONG
        std::cout<<"RA isDadRequired: " <<ra->isDadRequired() << endl;
        if (!ra->isDadRequired()) {
            //skip DAD
            Ipv6Address destAddr = packet->getTag<L3AddressInd>()->getSrcAddress().toIpv6();
        ...
        const Ipv6Address& dgSrcAddr, InterfaceEntry *ie)
{
    ...
    if (ns->isDadMsg() && neighbourCache.getDefaultRouterList().begin() == neighbourCache.getDefaultRouterList().end())
    {
        ns->setIsRelay(true);
    ...
    aro.setEUI64(ifid);
    ns->setARO(aro);
    EV_INFO<<"aro EUI64 low: "<<ns->getARO().getEUI64().low<< endl;
    EV_INFO<<"aro EUI64 len: "<<ns->getARO().getEUI64().len<< endl;
    EV_INFO<<"aro EUI64 normal: "<<ns->getARO().getEUI64().normal<< endl;
    ...
    EV_INFO<<"set mac src addr to: "<<ie->getMacAddress()<<endl;

    Neighbour *neighbourEntry = neighbourCache.lookup(dgDestAddr, ie->getInterfaceId());
    if (neighbourEntry!=nullptr) {
        packet->getTag<MacAddressReq>()->setDestAddress(neighbourEntry->macAddress);
        EV_INFO<<"set mac addr to: "<<packet->getTag<MacAddressReq>()->getDestAddress()<<endl;
    }
...
void Ipv6NeighbourDiscovery::processNsPacket(Packet *packet, const Ipv6NeighbourSolicitation *ns)
{
    ...
    EV_INFO<<"nsTargetAddr is: "<<nsTargetAddr<<endl;
    nsTargetAddr.setPrefix(Ipv6Address::LINKLOCAL_PREFIX, 64);
    ...
    else if (!rt6->isRouter()&& entry!=nullptr /* && ns->isDadMsg()*/) {

        // Forward multihop DAD message to RSU for neighbour
//        Ipv6Address destAddr = Ipv6Address("bbbb:0:64:0:8aa:ff:fe00:2");
...
//New messages for Multihop DAD   XIANG ZHONG
...
//-------------------------------------------------------------------------------------------//
    ...
    EV_INFO << "ie interfaceid: "<<ie->getInterfaceId()<< endl;
    EV_INFO << "ie interface name: "<<ie->getInterfaceName()<< endl;
...
void Ipv6NeighbourDiscovery::processDACPacket(Packet *packet, const Ipv6DuplicateAddrConfirm *dac)
{
    ...
    //XIANG ZHONG  get interfaceEntry by name
    InterfaceEntry *ie = ift->getInterfaceByName("nic");
    ...
    if (status == 0)
    {
        ...
        EV_INFO <<"ie name: "<< ie->getInterfaceName()<<endl;
        ...
        if (!neighbourEntry->relayAddr.isUnspecified())
            ...
            destAddr = dac->getRegisteredAddress();

        EV_INFO << "na destAddr is: " <<destAddr<< endl;
...
void Ipv6NeighbourDiscovery::processNsForMultihopDAD(Packet *packet, const Ipv6NeighbourSolicitation *ns, InterfaceEntry *ie)
{
    ...
    Ipv6Address MaAddress = Ipv6Address("aaaa:2:65:0:8aa:ff:fe00:9"); //destAddr
    ...
    if (entry == nullptr) {
        ...
        if (ns->isRelay())
        {
            Ipv6Address relayAddr= ns->getRelayAddress();
            neighbourCache.addRelayNeighbour(nsL3SrcAddr, ifID, nsMacAddr , ns->getARO().getRegistLifeTime(),relayAddr);
        }
        else
            neighbourCache.addTentativeNeighbour(nsL3SrcAddr, ifID, nsMacAddr , ns->getARO().getRegistLifeTime());
        ...
        scheduleAt(simTime() + ns->getARO().getRegistLifeTime()+ uniform(0.001,0.02), msg);

        //ns->getARO().getRegistLifeTime()
...
//        entry->routerExpiryTime = simTime() + ie->ipv6Data()->_getReachableTime();
        createAndSendDARPacket(nsTargetAddr, ns, MaAddress, sourceAddr, ie);
...
void Ipv6NeighbourDiscovery::forwardNDMessageForNeighbour(Packet *packet, const Ipv6Address& destAddr, InterfaceEntry *ie)
{
    ...
    Ipv6Address dgSrcAddr = ie->ipv6Data()->getAddress(0);
...
void Ipv6NeighbourDiscovery::processNsWithSpecifiedSrcAddr(Packet *packet, const Ipv6NeighbourSolicitation *ns, InterfaceEntry *ie)
{
    ...
    if (entry == nullptr) {
        ...
        if (!rt6->isRouter()) {
            ...
            neighbourCache.addRelayNeighbour(nsL3SrcAddr, ifID, nsMacAddr ,
...
void Ipv6NeighbourDiscovery::processNsForUpdateNCE(Packet *packet, const Ipv6NeighbourSolicitation *ns, InterfaceEntry *ie)
{
    ...
    //need to modify
        ...
        EV_INFO <<"nsMacAddr: "<<nsMacAddr << endl;

        /*If an entry already exists, and the cached link-layer address differs from
           the one in the received Source Link-Layer option,*/

        //problem isUnspecified
//        if (!(entry->macAddress.equals(nsMacAddr)) /*&& !nsMacAddr.isUnspecified()*/) {
...
void Ipv6NeighbourDiscovery::sendSolicitedNa(Packet *packet, const Ipv6NeighbourSolicitation *ns, InterfaceEntry *ie)
{
    ...
    // XIANG ZHONG change src address from global address to link local address
...
//XIANG ZHONG
void Ipv6NeighbourDiscovery::sendSolicitedNaForDAD(Packet *packet, Ipv6Address& destAddr, const Ipv6DuplicateAddrConfirm *dac, InterfaceEntry *ie)
{
    auto na = makeShared<Ipv6NeighbourAdvertisement>();
    na->setChunkLength(ICMPv6_HEADER_BYTES + B(IPv6_ADDRESS_SIZE));      // FIXME set correct length
...
//        naDestAddr = packet->getTag<L3AddressInd>()->getSrcAddress().toIpv6(); //global address
        //not sure
        Ipv6Address naDestAddr1 = dac->getRegisteredAddress();
//        naDestAddr = packet->getTag<L3AddressInd>()->getDestAddress().toIpv6();
        naDestAddr = dac->getRegisteredAddress();
        EV_INFO <<"L3AddressInd NA dest Addr is "<< naDestAddr1 <<endl;
...
//    Neighbour *neighbourEntry = neighbourCache.lookup(naTargetAddr, ie->getInterfaceId());
    Ipv6Address newAddr = naTargetAddr.setPrefix(Ipv6Address::LINKLOCAL_PREFIX, 64);
    Neighbour *neighbourEntry = neighbourCache.lookup(newAddr, ie->getInterfaceId());
    //XIANG ZHONG
    ...
    if (addrRegistEntry != nullptr) {
        ...
        addrRegistEntry->numNSSent =0;
        std::cout << "processNaPacket(): numNSSent: "<<addrRegistEntry->numNSSent << endl;
    }
    else
        EV_DETAIL << "addrRegistEntry is nullptr!" << endl;

//        ie->ipv6Data()->assignAddress(naTargetAddress, false, simTime() + entry.validLifetime,
//                        simTime() + entry.preferredLifetime, entry.hFlag);
        ie->ipv6Data()->assignAddress(naTargetAddr, false, simTime(),
                        simTime());
    }
    // forward NDMessage for neighbour
    else if (neighbourEntry!=nullptr && rt6->isLocalAddress(neighbourEntry->relayAddr)) {
        forwardNDMessageForNeighbour(packet, naTargetAddr, ie);
    }
        ...
        if (na->getPrefixInformation(0).getPrefix()!=Ipv6Address::LINKLOCAL_PREFIX) {
            processRaPrefixInfoForAddrAutoConf(na->getPrefixInformation(0), ie);
            //0304
//            initiateMultihopDad(ie->ipv6Data()->getLinkLocalAddress(),destAddr, ie);
            initiateMultihopDad(ie->ipv6Data()->getPreferredAddress(),destAddr, ie);
        ...
        EV_INFO <<"This is just information NA" << endl;
        AddrRegistEntry *addrRegistEntry = fetchAddrRegistEntry(ie);
            if (addrRegistEntry != nullptr) {
                EV_DETAIL << "addrRegistEntry is not nullptr, reset numSent!" << endl;
                addrRegistEntry->numNSSent =0;
...
void Ipv6NeighbourDiscovery::processRaPrefixInfoForAddrAutoConf(const Ipv6NdPrefixInformation& prefixInfo, InterfaceEntry *ie, bool hFlag)
{
    ...
    if ((isPrefixAssignedToInterface == false) && (validLifetime != 0)) {
        ...
        EV_INFO << "The received Prefix is:" <<prefix<< endl;
        Ipv6Address newAddr = linkLocalAddress.setPrefix(prefix, prefixLength);
        EV_INFO << "New address is:" <<newAddr<< endl;
        ...
        if (returnedHome) {
            ...
            if (ie->ipv6Data()->getNumAddresses() == 1) {
                ...
                //XIANG ZHONG
                //not sure if it is the right place to start DAD
                if (neighbourCache.getDefaultRouterList().begin() != neighbourCache.getDefaultRouterList().end())
                {
                    ...
                    std::cout << "MultihopDAD dest address is: "<< destAddr <<endl;
//                    Ipv6Address destAddr = Ipv6Address("fe80::8aa:ff:fe00:2");
