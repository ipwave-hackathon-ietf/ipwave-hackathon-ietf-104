...
//    enum ReachabilityState { INCOMPLETE, REACHABLE, STALE, DELAY, PROBE };
    ...
    //XIANG ZHONG
    //add new NCE states: Tentative, Registered, Garbage-collectible
    enum ReachabilityState { INCOMPLETE, REACHABLE, STALE, DELAY, PROBE ,TENTATIVE, REGISTERED, GARBAGE  };
    ...
    struct Neighbour
    {
        ...
        //XIANG ZHONG
        //registration time
        simtime_t registLifeTime;
        Ipv6Address relayAddr;
  ...
  public:
    ...
    //XIANG ZHONG
//    MultihopDadMap::iterator begin() { return multihopDadMap.begin(); }
//    /** For iteration on the internal std::map */
//    MultihopDadMap::iterator end() { return multihopDadMap.end(); }
    ...
    //XIANG ZHONG
    virtual Neighbour *addTentativeNeighbour(const Ipv6Address& addr, int interfaceID, MacAddress macAddress, short registLifetime);
    virtual Neighbour *addRelayNeighbour(const Ipv6Address& addr, int interfaceID, MacAddress macAddress, short registLifetime, const Ipv6Address& relayAddr);
    //Multihop DAD Table
    virtual Neighbour *addMultihopNCE(const Ipv6Address& addr, int interfaceID);
    virtual Neighbour *lookupDadTable(const Ipv6Address& addr, int interfaceID);
    // END
    ...
    //XIANG ZHONG
    virtual void removeFromDadTable(const Ipv6Address& addr, int interfaceID);
    ...
    // XIANG ZHONG
//    virtual void remove(MultihopDadMap::iterator it);
