...
Ipv6NeighbourCache::Neighbour *Ipv6NeighbourCache::addTentativeNeighbour(const Ipv6Address& addr, int interfaceID, MacAddress macAddress, short registLifetime)
{
    Key key(addr, interfaceID);
    ASSERT(neighbourMap.find(key) == neighbourMap.end());    // entry must not exist yet
    Neighbour& nbor = neighbourMap[key];
...
Ipv6NeighbourCache::Neighbour *Ipv6NeighbourCache::addRelayNeighbour(const Ipv6Address& addr, int interfaceID, MacAddress macAddress, short registLifetime, const Ipv6Address& relayAddr)
{
    Key key(addr, interfaceID);
    ASSERT(neighbourMap.find(key) == neighbourMap.end());    // entry must not exist yet
    Neighbour& nbor = neighbourMap[key];
...
//void Ipv6NeighbourCache::removeFromDadTable(NeighbourMap::iterator it)
//{
